...
COMS 회원용 모바일 앱 — Android · iOS · iPadOS.

COMS 회원이 휴대폰에서 매일 쓰는 기능만 모은 독립 앱입니다.
...
가장 쉬운 방법입니다. 폰 브라우저에서 바로 받을 수 있습니다.
...
iOS는 Android APK처럼 파일 하나를 공개 링크로 바로 설치할 수 없습니다. Personal Team 서명은 본인 Apple ID에 연결된 기기만 통과합니다. 본인 Mac + iPhone만 있으면 직접 설치할 수 있습니다.
...
3. Capacitor iOS 프로젝트 동기화:
   ...
   `npm run cap:ios`가 `ios/App/App.xcodeproj`를 Xcode로 엽니다. 이 프로젝트는 저장소에 이미 포함되어 있어서 `xcodegen generate`는 필요 없습니다.

4. Xcode에서:

   - 좌측 `App` 타깃 선택
   - `Signing & Capabilities` - `Team`에서 본인 Apple ID 선택 (Personal Team)
   - iPhone 연결
   - 좌측 상단 디바이스 드롭다운에서 본인 iPhone 선택
   - Run 버튼 또는 `Cmd+R`
...
최소 요구사항: macOS + Xcode 16 이상, iOS 15.0 이상.

무료 Apple ID로 설치한 앱은 보통 7일마다 재서명이 필요합니다. 여러 회원에게 배포하려면 TestFlight가 현실적인 경로입니다. iOS 푸시 토큰 등록은 실제 기기, APNs 설정, 서버의 `/api/mobile/v1/push-tokens` 동작이 모두 필요합니다.

자세한 단계 + 7일 재서명 사이클 + 무료 ID 한계는 [`ios/INSTALL_PERSONAL_DEVICE.md`](ios/INSTALL_PERSONAL_DEVICE.md) 참고.

### 🧪 iOS TestFlight → 여러 회원에게 배포
